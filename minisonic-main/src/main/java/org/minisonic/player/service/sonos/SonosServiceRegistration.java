...
public class SonosServiceRegistration {
    ...
    public void setEnabled(String minisonicBaseUrl, String sonosControllerIp, boolean enabled, String sonosServiceName, int sonosServiceId) throws IOException {
        String localUrl = minisonicBaseUrl + "ws/Sonos";
        ...
        LOG.info((enabled ? "Enabling" : "Disabling") + " Sonos music service, using Sonos controller IP " + sonosControllerIp +
                 ", SID " + sonosServiceId + ", and Minisonic URL " + localUrl);
        ...
        if (enabled) {
            ...
            params.add(Pair.create("presentationMapUri", minisonicBaseUrl + "sonos/presentationMap.xml"));
            params.add(Pair.create("stringsVersion", "5"));
            params.add(Pair.create("stringsUri", minisonicBaseUrl + "sonos/strings.xml"));
        }
